...
/******************************************************************************************************/
/******************************************************************************************************/
void LeggedController::updateAdaptiveParams(const vector_t& state, const vector_t& stateDes,
                                            const ros::Duration& period) {
  using namespace ocs2::legged_robot::adaptive;

  // Get shared adaptive state from LeggedInterface (also used by MPC CLF constraint)
  auto& params = leggedInterface_->getAdaptiveParams();

  const scalar_t dt = period.toSec();
  if (dt <= 0.0) return;

  // ---- Compute composite error σ = ṽ + Λ * q̃ ----
  vector3_t v_current = state.segment<3>(0);
  vector3_t v_desired = stateDes.segment<3>(0);
  vector3_t v_tilde = v_current - v_desired;

  vector3_t p_current = state.segment<3>(6);
  vector3_t p_desired = stateDes.segment<3>(6);
  vector3_t p_tilde = p_current - p_desired;

  vector3_t sigma_l = v_tilde + params.Lambda_l * p_tilde;

  vector3_t omega_current = state.segment<3>(3);
  vector3_t omega_desired = stateDes.segment<3>(3);
  vector3_t omega_tilde = omega_current - omega_desired;

  vector3_t euler_current = state.segment<3>(9);
  vector3_t euler_desired = stateDes.segment<3>(9);
  euler_current(0) = euler_desired(0) + angles::shortest_angular_distance(euler_desired(0), euler_current(0));
  vector3_t euler_tilde = euler_current - euler_desired;

  vector3_t sigma_o = omega_tilde + params.Lambda_o * euler_tilde;

  vector_t sigma(6);
  sigma << sigma_l, sigma_o;

  // ---- Compute regressor Y_u ----
  vector3_t v_dot_pr = vector3_t::Zero();
  vector3_t omega_dot_r = vector3_t::Zero();
  vector3_t omega = omega_current;
  vector3_t gravity(0.0, 0.0, -9.81);

  matrix_t Yu = AdaptiveRegressor::computeRegressor(v_dot_pr, omega, omega_dot_r, omega, gravity);

  // ---- Update parameters: π̂_u += Γ * Y_u^T * σ * dt ----
  params.pi_hat += params.Gamma * Yu.transpose() * sigma * dt;

  // Compute adaptive wrench for WBC compensation
  vector3_t f_u = params.computeAdaptiveForce(v_dot_pr, gravity);
  vector3_t t_u = params.computeAdaptiveTorque(omega, v_dot_pr);
  leggedInterface_->setAdaptiveWrench(f_u, t_u);

  // Log adaptive state periodically (every 1s)
